...
def sampleSentences(word, count=1, action_corpus=current_actions):
    #sents = getSentences(word)
    prob = sourcing.findNearbyVerbs(word, action_corpus)
    if len(prob) > 100:
        prob = prob[:100]
    ...
    select = numpy.random.choice(sp, 15, p=pl, replace=False)
    return select #prob[select][0]
    #return sourcing.findVerbCloseness(word, current_actions)
    
def displayActions(action_list):
...
def writeNouns(corpus, filename="noun_list.txt"):
    nouns = list(nounChunks(corpus))
    textacy.fileio.write_file_lines(nouns, filename, encoding="utf8")

def writeVerbs(actions, filename="verb_list.txt"):
    verbs = set()
    for a in actions:
        if a.primeverb:
            verbs = verbs.union(set([a.primeverb.lemma_]))
    textacy.fileio.write_file_lines(verbs, filename, encoding="utf8")

def writeSentences(actions, filename = "sent_list.txt"):
